// import { graph } from '@/services';
import { downloadFile } from 'utils/snark';
import { poolAbi as InstanceABI } from 'contracts';
import { Contract, ethers, EventFilter } from 'ethers';
import { EventServiceArgs } from './types';
import { formatEvents } from 'utils/pool';
import { sleep } from 'utils/time';
import { InstanceInfo } from 'types';
import { Instance } from 'config/network';

type EventType = 'commitment' | 'nullifier';

class EventService {
  chainId: number;
  token: string;
  factoryMethods: Record<string, Function>;
  contract: Contract;
  instance: Instance;

  constructor({ chainId, token, factoryMethods, instance }: EventServiceArgs) {
    // this.idb = window.$nuxt.$indexedDB(netId);
    this.chainId = chainId;
    this.token = token;

    this.factoryMethods = factoryMethods as any;
    this.instance = instance;
    this.contract = this.getContract();

    // this.isNative = nativeCurrency === this.currency;
    // this.hasCache = this.isNative && (Number(this.netId) === 1 || Number(this.netId) === 56);
  }

  getInstanceName(type: EventType) {
    return `${type}s_${this.token}`;
  }

  async getEvents(type: EventType) {
    // let cachedEvents = await this.getEventsFromDB(type);

    // if (!cachedEvents && this.hasCache) {
    //   cachedEvents = await this.getEventsFromCache(type);
    // }
    const cachedEvents = await this.getEventsFromCache(type);
    return cachedEvents;
  }

  async updateEvents(type: EventType, cachedEvents?: any) {
    const { deployedBlock } = this.instance;

    const savedEvents = cachedEvents || (await this.getEvents(type));

    let fromBlock = deployedBlock;
    if (savedEvents) {
      fromBlock = savedEvents.lastBlock + 1;
    }

    const newEvents = await this.getEventsFromBlock(
      type,
      fromBlock,
      // graphMethod: `getAll${capitalizeFirstLetter(type)}s`,
    );

    const allEvents = []
      .concat(savedEvents?.events || [], newEvents?.events || ([] as any))
      .sort((a: any, b: any) => {
        if (a.leafIndex && b.leafIndex) {
          return a.leafIndex - b.leafIndex;
        }
        return a.blockNumber - b.blockNumber;
      });

    const lastBlock =
      allEvents.length > 0 ? (allEvents[allEvents.length - 1] as any).blockNumber : deployedBlock;

    // this.saveEvents({ events: allEvents, lastBlock, type });

    return {
      events: allEvents,
      lastBlock,
    };
  }

  // async findEvent({ eventName, eventToFind, type }) {
  //   const instanceName = this.getInstanceName(type);

  //   let event = await this.idb.getFromIndex({
  //     storeName: instanceName,
  //     indexName: eventName,
  //     key: eventToFind,
  //   });

  //   if (event) {
  //     return event;
  //   }

  //   const savedEvents = await this.getEvents(type);
  //   if (savedEvents) {
  //     event = savedEvents.events.find((event) => event[eventName] === eventToFind);
  //     if (event) {
  //       return event;
  //     }
  //   }

  //   const freshEvents = await this.updateEvents(type);
  //   event = freshEvents && freshEvents?.events.find((event) => event[eventName] === eventToFind);

  //   return event;
  // }

  getContract() {
    return this.factoryMethods.getContract(this.instance.instanceAddress);
  }

  async getEventsFromCache(type: EventType) {
    try {
      const instanceName = this.getInstanceName(type);

      const content = await downloadFile({
        contentType: 'string',
        name: `events/${this.chainId}/${instanceName}.json.zip`,
      });

      if (content) {
        const events = JSON.parse(content);

        return {
          events,
          lastBlock: events[events.length - 1].blockNumber,
        };
      }

      return {
        events: [],
        lastBlock: '',
      };
    } catch (err) {
      return undefined;
    }
  }

  // async getEventsFromDB(type) {
  //   try {
  //     const instanceName = this.getInstanceName(type);

  //     const savedEvents = await this.idb.getAll({ storeName: instanceName });

  //     if (!savedEvents || !savedEvents.length) {
  //       return undefined;
  //     }

  //     const event = await this.idb.getFromIndex({
  //       storeName: 'lastEvents',
  //       indexName: 'name',
  //       key: instanceName,
  //     });

  //     return {
  //       events: savedEvents,
  //       lastBlock: event.blockNumber,
  //     };
  //   } catch (err) {
  //     return undefined;
  //   }
  // }

  // async getStatisticsRpc({ eventsCount }) {
  //   const { deployedBlock } = networkConfig[`netId${this.netId}`];
  //   const savedEvents = await this.getEvents(eventsType.DEPOSIT);

  //   if (savedEvents.events.length) {
  //     const { events } = await this.updateEvents(eventsType.DEPOSIT, savedEvents);
  //     return events;
  //   }

  //   const blockRange = 4950;
  //   const fromBlock = deployedBlock;
  //   const { blockDifference, currentBlockNumber } = await this.getBlocksDiff({ fromBlock });

  //   let numberParts = blockDifference === 0 ? 1 : Math.ceil(blockDifference / blockRange);
  //   const part = Math.ceil(blockDifference / numberParts);

  //   let events = [];
  //   let toBlock = currentBlockNumber;

  //   if (fromBlock < currentBlockNumber) {
  //     for (let i = 0; i < numberParts; i++) {
  //       try {
  //         await sleep(200);
  //         const partOfEvents = await this.getEventsPartFromRpc({
  //           fromBlock: toBlock - part,
  //           toBlock,
  //           type: eventsType.DEPOSIT,
  //         });

  //         if (partOfEvents) {
  //           events = events.concat(partOfEvents.events);
  //           if (eventsCount <= events.length) {
  //             break;
  //           }
  //         }
  //         toBlock -= part;
  //       } catch {
  //         numberParts = numberParts + 1;
  //       }
  //     }
  //     if (eventsCount !== events.length) {
  //       const savedEvents = await this.getEvents(eventsType.DEPOSIT);
  //       events = events.concat(savedEvents?.events || []);
  //     }
  //   }

  //   return events;
  // }

  // async getEventsFromGraph({ fromBlock, methodName }) {
  //   try {
  //     const { events, lastSyncBlock } = await graph[methodName]({
  //       fromBlock,
  //       netId: this.netId,
  //       amount: this.amount,
  //       currency: this.currency,
  //     });
  //     return {
  //       events,
  //       lastBlock: lastSyncBlock,
  //     };
  //   } catch (err) {
  //     return undefined;
  //   }
  // }

  async getBlocksDiff(fromBlock: number) {
    const currentBlockNumber = await this.factoryMethods.getBlockNumber();

    return {
      currentBlockNumber,
      blockDifference: Math.ceil(currentBlockNumber - fromBlock),
    };
  }

  async getEventsPartFromRpc(type: EventType, fromBlock: number, toBlock?: number) {
    try {
      const { currentBlockNumber } = await this.getBlocksDiff(fromBlock);

      if (fromBlock > currentBlockNumber) {
        return {
          events: [],
          lastBlock: fromBlock,
        };
      }

      let eventFilter: EventFilter;
      if (type === 'commitment') {
        eventFilter = this.contract.filters.NewCommitment();
      } else if (type === 'nullifier') {
        eventFilter = this.contract.filters.NewNullifier();
      } else {
        throw new Error(`Event type ${type} unknown`);
      }

      const events = await this.contract.queryFilter(eventFilter, fromBlock, toBlock || 'latest');

      if (!events?.length) {
        return {
          events: [],
          lastBlock: fromBlock,
        };
      }
      return {
        events: formatEvents(events, type),
        lastBlock: events[events.length - 1].blockNumber,
      };
    } catch (err) {
      return undefined;
    }
  }

  async getBatchEventsFromRpc(type: EventType, fromBlock: number) {
    try {
      const blockRange = 4950;
      const { blockDifference, currentBlockNumber } = await this.getBlocksDiff(fromBlock);

      let numberParts = blockDifference === 0 ? 1 : Math.ceil(blockDifference / blockRange);
      const part = Math.ceil(blockDifference / numberParts);

      let events: any[] = [];
      let toBlock: number = fromBlock + part;

      if (fromBlock < currentBlockNumber) {
        if (toBlock >= currentBlockNumber) {
          // toBlock = 'latest';
          toBlock = currentBlockNumber;
          numberParts = 1;
        }

        for (let i = 0; i < numberParts; i++) {
          try {
            await sleep(200);
            const partOfEvents = await this.getEventsPartFromRpc(type, fromBlock, toBlock);
            if (partOfEvents) {
              events = events.concat(partOfEvents.events);
            }
            fromBlock = toBlock;
            toBlock += part;
          } catch {
            numberParts = numberParts + 1;
          }
        }
        if (events.length) {
          return {
            events,
            lastBlock: toBlock,
            // lastBlock: toBlock === 'latest' ? currentBlockNumber : toBlock,
          };
        }
      }
      return undefined;
    } catch (err) {
      return undefined;
    }
  }

  async getEventsFromRpc(type: EventType, fromBlock: number): Promise<any[]> {
    try {
      let events;

      if (Number(this.chainId) === 56) {
        const rpcEvents = await this.getBatchEventsFromRpc(type, fromBlock);
        events = rpcEvents?.events || [];
      } else {
        const rpcEvents = await this.getEventsPartFromRpc(type, fromBlock);
        events = rpcEvents?.events || [];
      }
      return events;
    } catch (err) {
      return [];
    }
  }

  // async getEventsFromBlock({ fromBlock, graphMethod, type }) {
  async getEventsFromBlock(type: EventType, fromBlock: number) {
    try {
      // ToDo think about undefined
      // const graphEvents = await this.getEventsFromGraph({ fromBlock, methodName: graphMethod });
      // const lastSyncBlock = fromBlock > graphEvents?.lastBlock ? fromBlock : graphEvents?.lastBlock;
      // const rpcEvents = await this.getEventsFromRpc({ fromBlock: lastSyncBlock, type });

      // const allEvents = [].concat(graphEvents?.events || [], rpcEvents || []);
      const allEvents = await this.getEventsFromRpc(type, fromBlock);
      if (allEvents.length) {
        return {
          events: allEvents,
          lastBlock: allEvents[allEvents.length - 1].blockNumber,
        };
      }
      return undefined;
    } catch (err) {
      return undefined;
    }
  }

  // async saveEvents({ events, lastBlock, type }) {
  //   try {
  //     if (!events || !events.length || this.idb.isBlocked) {
  //       return;
  //     }

  //     const instanceName = this.getInstanceName(type);

  //     await this.idb.createMultipleTransactions({
  //       data: events,
  //       storeName: instanceName,
  //     });

  //     await this.idb.putItem({
  //       data: {
  //         blockNumber: lastBlock,
  //         name: instanceName,
  //       },
  //       storeName: 'lastEvents',
  //     });
  //   } catch (err) {
  //     console.error('saveEvents has error:', err.message);
  //   }
  // }
}

class EventsFactory {
  provider: ethers.providers.JsonRpcProvider;
  instances = new Map();

  constructor(rpcUrl: string) {
    this.provider = new ethers.providers.JsonRpcProvider(rpcUrl);
  }

  getBlockNumber = () => {
    return this.provider.getBlockNumber();
  };

  getContract = (address: string): Contract => {
    return new ethers.Contract(address, InstanceABI, this.provider);
  };

  getService = (info: InstanceInfo) => {
    const instanceName = `${info.chainId}/${info.token}`;
    if (this.instances.has(instanceName)) {
      return this.instances.get(instanceName);
    }

    const instance = new EventService({
      ...info,
      factoryMethods: {
        getContract: this.getContract,
        getBlockNumber: this.getBlockNumber,
      },
    });
    this.instances.set(instanceName, instance);
    return instance;
  };
}

export { EventsFactory };
